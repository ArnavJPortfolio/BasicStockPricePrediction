...
ticker_dict = {}
loc = r"C:\Users\arnav\Downloads\nasdaq_screener_1617226118306.xls"
...
for i in range(1, sheet.nrows):
    ticker_dict[sheet.cell_value(i, 0)] = sheet.cell_value(i, 1)


# Determine if Ticker Exists then add to search
...
while True:
     try:
        num_tickers = int(input("Enter number of tickers you want to input: "))
        break
     except ValueError:
         print("That was not a valid number. Try again...")
for num in range(num_tickers):
    ticker = input("Input Company Name or Ticker Symbol: ")
    # Check for validity
    if len(ticker) < 1 or hasNumbers(ticker):
        while hasNumbers(ticker):
            print("Please Try Again as that Name or Symbol is invalid or may contain a number")
            ticker = input("Input Company Name or Ticker Symbol: ")
    elif ticker.upper() in list_of_tickers or ticker in list_of_tickers:
        while ticker.upper() in list_of_tickers or ticker in list_of_tickers:
            print("Please Try Again as that Name or Symbol has already been added to the portfolio")
            ticker = input("Input Company Name or Ticker Symbol: ")
    elif ticker.upper() not in list(ticker_dict.keys()) and ticker not in list(ticker_dict.keys()):
        while ticker.upper() not in list(ticker_dict.keys()) or ticker not in list(ticker_dict.keys()):
            print("Please Try Again as that Name or Symbol does not exist")
            ticker = input("Input Company Name or Ticker Symbol: ")
    for symbol, name in ticker_dict.items():
        if ticker.lower() in name.lower():
            if name in list_of_tickers or symbol in list_of_tickers:
                print("Please Try Again as that Name or Symbol has already been added to the portfolio")
                ticker = input("Input Company Name or Ticker Symbol: ")
    # Input into Portfolio List based on symbol or name
    if len(ticker) > 5:
        for symbol, name in ticker_dict.items():
            if ticker.lower() in name.lower():
                num_stocks = 0
                while True:
                    try:
                        num_stocks = int(input("Number of Stocks: "))
                        break
                    except ValueError:
                        print("That was not a valid number. Try again...")
                list_of_tickers.append({symbol: num_stocks})
        print("Current Stocks in Portfolio: ", list_of_tickers)
    else:
        tick = difflib.get_close_matches(ticker.upper(), list(ticker_dict.keys()))
        num_stocks = 0
        while True:
            try:
                num_stocks = int(input("Number of Stocks: "))
                ...
                print("That was not a valid number. Try again...")
        list_of_tickers.append({tick[0]: num_stocks})
        print("Current Stocks in Portfolio: ", list_of_tickers)
...
symbollist = [list(tickers.keys())[0] for tickers in list_of_tickers]
symbol2 = "^IXIC"
gainerdata = []
loserdata = []
activedata = []
...
def updateDailies():
    global stockOne, stockTwo, loserbox, gainerbox, activitybox, count
    # set the current price label to the current price
    currPrice1.set("Current Price: $" + (str(round(float(si.get_live_price(symbollist[count])), 2))))
